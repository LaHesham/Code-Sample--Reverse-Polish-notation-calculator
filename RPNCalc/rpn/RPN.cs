...
namespace RPNCalc.rpn
{
	...
	public class RPN
	{
		...
		public void Parse()
		{
			...
			for (int i = 0; i <= expression.Length-1; i++)
			{
				...
				{ 
					...
					else if (rawToken == "~") tokenQueue.Enqueue(new Token(TokenType.UnaryMinus));
		...
		private string GetOperator(string currentSymbol, ref int pos, ref int call)
		{
			...
			if (currentSymbol == "-" && (pos - 1) < 0)
			{
				return "~";
			}
			else if (currentSymbol == "-" && expression[pos-1].ToString() == "(")
			{
				return "~";
			}
			else if (currentSymbol == "-" && expression[pos - 1].ToString() == "/")
			...
			else if (currentSymbol == "-" && !((pos - 1) < 0 || expression[pos - 1].ToString() == "(" ||
				expression[pos - 1].ToString() == "/" || expression[pos - 1].ToString() == "*"))
			{
		...
		public void BuildRPNQUeue()
		{
			...
			while (tokenQueue.Count !=0 )
			{
				...
				if (!tkn.IsOperator)
				{
					if (nextIsNegative)
					{
						tkn.PTokenValue = tkn.PTokenValue * -1;
				...
				else if (operatorStack.Count == 0 && tkn.PTokenType != TokenType.UnaryMinus) operatorStack.Push(tkn);
				else if (tkn.PTokenType == TokenType.UnaryMinus)
				{
					nextIsNegative = true;
				}
				...
				else if (operatorStack.Peek().OperationPriority > tkn.OperationPriority)
				{
					...
					else if (operatorStack.Peek().OperationPriority == 4 || operatorStack.Peek().PTokenType == TokenType.SquareRoot)
					{
						rpnQueue.Enqueue(operatorStack.Pop());
						operatorStack.Push(tkn);
					}
